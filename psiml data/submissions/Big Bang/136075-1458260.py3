...
def find_world_hits(coords, velocities, N, S, T, P):
    ...
    while np.any(wall_bangs != old_wall_bangs):
        ...
        count += np.count_nonzero(1*np.all(new_starting_points != 0, axis=1))
        old_wall_bangs = wall_bangs.copy()
        wall_bangs = wall_bangs + np.logical_not(np.all(np.abs(coords + starting_points) < S, axis=1))
