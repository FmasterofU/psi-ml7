...
def find_world_hits(coords, velocities, S, T, P):
    ...
    while np.any(wall_bangs != old_wall_bangs):
        new_angles = - angles * np.where(np.abs(starting_points[:, 1]) == S, 1, 0).reshape((2,1)) + \
            (np.pi - angles) * np.where(np.abs(starting_points[:, 0]) == S, 1, 0).reshape((2,1)) * (angles >= 0).reshape((2,1)) + \
            (-np.pi - angles) * np.where(np.abs(starting_points[:, 0]) == S, 1, 0).reshape((2,1)) * (angles < 0).reshape((2,1))
        ...
        x_plus_S = 1*(starting_points[:, 0] != S) * np.hstack((
        ...
        x_plus_S = x_plus_S * np.logical_and(x_plus_S[:, 1] <= S, x_plus_S[:, 1] >= -S) * np.repeat((np.linalg.norm(x_plus_S - starting_points, ord=2, axis=1) <= np.linalg.norm(coords, ord=2, axis=1)).reshape((angles.shape[0],1)), 2, axis=1)
        x_minus_S = 1*(starting_points[:, 0] != -S) * np.hstack((
            (-S * np.ones(angles.shape[0])).reshape((angles.shape[0], 1)),
            (np.tan(angles).reshape(angles.shape[0]) * (-S - coords[:, 0] - starting_points[:, 0]) + coords[:, 1] + starting_points[:, 1]).reshape((angles.shape[0], 1))
        ))
        x_minus_S = x_minus_S * np.logical_and(x_minus_S[:, 1] <= S, x_minus_S[:, 1] >= -S) * np.repeat((np.linalg.norm(x_minus_S - starting_points, ord=2, axis=1) <= np.linalg.norm(coords, ord=2, axis=1)).reshape((angles.shape[0],1)), 2, axis=1)
        y_plus_S = 1*(starting_points[:, 1] != S) * np.hstack((
            ((S - coords[:, 1] - starting_points[:, 1]) / np.tan(angles).reshape(angles.shape[0]) + coords[:, 0] + starting_points[:, 0]).reshape((angles.shape[0], 1)),
            (S * np.ones(angles.shape[0])).reshape((angles.shape[0], 1))
        ))
        y_plus_S = y_plus_S * np.logical_and(y_plus_S[:, 0] <= S, y_plus_S[:, 0] >= -S) * np.repeat((np.linalg.norm(y_plus_S - starting_points, ord=2, axis=1) <= np.linalg.norm(coords, ord=2, axis=1)).reshape((angles.shape[0],1)), 2, axis=1)
        y_minus_S = 1*(starting_points[:, 1] != -S) * np.hstack((
            ((-S - coords[:, 1] - starting_points[:, 1]) / np.tan(angles).reshape(angles.shape[0]) + coords[:, 0] + starting_points[:, 0]).reshape((angles.shape[0], 1)),
            (-S * np.ones(angles.shape[0])).reshape((angles.shape[0], 1))
        ))
        y_minus_S = y_minus_S * np.logical_and(y_minus_S[:, 0] <= S, y_minus_S[:, 0] >= -S) * np.repeat((np.linalg.norm(y_minus_S - starting_points, ord=2, axis=1) <= np.linalg.norm(coords, ord=2, axis=1)).reshape((angles.shape[0],1)), 2, axis=1)
    ...
    return int(np.sum(wall_bangs)), np.sum(np.power(P, wall_bangs))
